...
import { getProductCartQuantitytt } from "../../helpers/product";

const Checkout = ({ location, cartItems, currency, products }) => {
  ...
  const getUserById = () => {
    axios
      .get("http://localhost:3200/api/get_client_byId/" + CurrentUser.userId)
      .then((res) => {
  ...
  const addOrders = () => {
    ...
    axios
      .post("http://localhost:3200/api/add_order", data)
      .then((res) => {
            ...
            if (products[i].variation[j].color === data.selectedProductColor) {
              for (let k = 0; k < products[i].variation[j].size.length; k++) {
                if (
                  ...
                  data.selectedProductSize &&products[i].variation[j].size[k].stock !==0
                ) {
